			...
			$http({method: 'jsonp', url: url}).
				success(function(data, status) {
					...
					//	$location.path('/main');
						...
						if(data.poll !== null){
							location.href='http://www.mycontact.co.il/mobile/#/poll/'+data.poll;
						} else {
							location.reload();
						}
					} else {
		...
		$http({method: 'jsonp', url: url, cache: true}).
			success(function(data, status) {
				...
				if(typeof data !== 'undefined' && data.length > 0){
					...
					angular.element(document).ready(function () {
							// var  TargetDate = $filter('date')(data[0].end, "MM/dd/yyyy");;
							// var  CountStepper = -1;
							// var SetTimeOutPeriod = (Math.abs(CountStepper)-1)*1000 + 990;
							// var dthen = new Date(TargetDate);
							// var dnow = new Date();
		...
		$http({method: 'jsonp', url: url, cache: true}).
			success(function(data, status) {
				...
				if(data !== 0){
					$scope.data = data;
					angular.element(document).ready(function () {
							var  TargetDate = $filter('date')(data.end, "MM/dd/yyyy");;
							var  CountStepper = -1;
							var SetTimeOutPeriod = (Math.abs(CountStepper)-1)*1000 + 990;
							var dthen = new Date(TargetDate);
							var dnow = new Date();
		...
		$http({method: 'jsonp', url: url, cache: true}).
			success(function(data, status) {
				...
				if(data.knoledge.id !== null){
					$scope.data = data;
					// $scope.time = secondsToHms(data.knoledge.microtime);
				...
				angular.element(document).ready(function () {
					// var  TargetDate = $filter('date')(data.assign.end_date, "MM/dd/yyyy") + ' 5:00 AM';
					// var  CountStepper = -1;
					// var SetTimeOutPeriod = (Math.abs(CountStepper)-1)*1000 + 990;
					// var dthen = new Date(TargetDate);
					// var dnow = new Date();
...
phmControllers.controller('LgFilterCtrl', [ '$scope', '$rootScope', 'userState', '$location', '$http', '$filter',
	function($scope, $rootScope, userState, $location, $http, $filter) {
		...
		var url = Jbase+'index.php?option=com_phm&view=filter&layout=json&tmpl=app&Itemid=188&callback=JSON_CALLBACK';
		...
		$http({method: 'jsonp', url: url, cache: true}).
			success(function(data, status) {
				...
				if(data.done === 1 && data.filter_results.id !== null){					
					$scope.data = data;
					// $scope.time = secondsToHms(data.filter_results.microtime);
	...
	function($scope, $rootScope, userState, $location, $http, $filter) {
		...
		$http({method: 'jsonp', url: url, cache: true}).
			success(function(data, status) {
				$rootScope.dispOverlay = 'hide';
				if(data.done === 1 && data.filter_results.id !== null){					
					// var time = secondsToHms(data.filter_results.microtime);
...
phmControllers.controller('catelogueCtrl', [ '$scope', '$rootScope', 'userState', '$location', '$http',
	function($scope, $rootScope, userState, $location, $http) {
		...
		angular.element(document).ready(function () {
			// $(".item_title").fitText(0.9, { minFontSize: 12, maxFontSize: 15 });		
		});
		$scope.openMenu = false;
    $scope.toggleMenu = function () {
        $scope.$emit('toggleSlideMenu', $scope.openMenu = !$scope.openMenu);
